...
class GptDataset(Dataset):
    ...
    def __init__(self,x_encoded,y_encoded,x_augment_encoded,num_turns=5):
        self.x_encoded = x_encoded
        self.y_encoded = y_encoded
        self.x_augment_encoded = x_augment_encoded
    ...
    def __getitem__(self,index):
        ...
        ref_start, speaker1,speaker2,augment,eos = 2,3,4,5,50256
        
        # augment
        x += [augment]
        for i in range(5):
            x += self.x_augment_encoded[index*self.num_turns+i]
        type_x = [augment]*len(x)
...
def main():
    ...
    x_augment = pickle.load(open(folder+'x_augment','rb'))
    x_augment_flat = []
    for x in x_augment:
        for i in x:
            x_augment_flat.append(i)
    ...
    x_augment_encoded = [tokenizer.encode(text_standardize(x)) for x in x_augment_flat]
    ...
    gpt_data = GptDataset(x_encoded,y_encoded,x_augment_encoded)
