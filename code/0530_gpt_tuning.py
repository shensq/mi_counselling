...
class GptDataset(Dataset):
    ...
    def __getitem__(self,index):
        ...
        x += [4] + self.x_encoded[index*self.num_turns]
        type_x += [4]*(len(self.x_encoded[index*self.num_turns])+1)
        
        x += [5] + self.x_encoded[index*self.num_turns+1]
        type_x += [5]*(len(self.x_encoded[index*self.num_turns+1])+1)
        
        x += [4] + self.x_encoded[index*self.num_turns+2]
        type_x += [4]*(len(self.x_encoded[index*self.num_turns+2])+1)
        
        x += [5] + self.x_encoded[index*self.num_turns+3]
        type_x += [4]*(len(self.x_encoded[index*self.num_turns+3])+1)
        
        x += [4] + self.x_encoded[index*self.num_turns+4]
        type_x += [4]*(len(self.x_encoded[index*self.num_turns+4])+1)
        lm_x += [-1]*len(x)
        
        
        x += [3] + self.y_encoded[index] + [50256]
        type_x += [3]*(len(self.y_encoded[index])+2)
...
def main():
    ...
    print(args)
    # ====== Load GPT2 model ========
    model = GPT2LMHeadModel.from_pretrained('/home/shensq/gpt_tuning/345M_after_Alex/')
    if USE_CUDA:
        model.cuda()
    tokenizer = GPT2Tokenizer.from_pretrained('/home/shensq/gpt_tuning/345M_after_Alex/')
